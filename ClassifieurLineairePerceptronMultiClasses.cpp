...
PerceptronMultiClasses::PerceptronMultiClasses(int inputSize, int numClasses, float lr) : inputSize(inputSize), numClasses(numClasses), learningRate(lr)
{
...
extern "C"
{
	...
	void train_perceptron_multiclasses
	(
		PerceptronMultiClasses *model, float *X,
		int *Y, int rows, int cols, int epochs
	)
	{
		if(!model)
		{
	...
	int get_input_size(PerceptronMultiClasses *model)
	{
		if(!model)
		{
			return 0;
	...
	int get_num_classes(PerceptronMultiClasses *model)
	{
		if(!model)
		{
			return 0;
	...
	void get_weights(PerceptronMultiClasses *model, float *outWeights)
	{
		if(!model || !outWeights)
		{
			return;
		...
		for(int c = 0 ; c < model->getNumClasses() ; ++c)
		{
			for(int i = 0 ; i < model->getInputSize() ; ++i)
			{
				outWeights[idx++] = weights[c][i];
			}
		}
	}

	void get_bias(PerceptronMultiClasses *model, float *outBias)
	{
		if(!model || !outBias)
		{
			return;
		}
		const auto &bias = model->getBias();
		for(int c = 0 ; c < model->getNumClasses() ; ++c)
		{
			outBias[c] = bias[c];
		}
	...
	void set_weights(PerceptronMultiClasses *model, float *inWeights)
	{
		if(!model || !inWeights)
		{
			return;
		}
		std::vector<std::vector<float>> weights(model->getNumClasses());
		int idx = 0;
		for(int c = 0 ; c < model->getNumClasses() ; ++c)
		{
			weights[c].resize(model->getInputSize());
			for(int i = 0 ; i < model->getInputSize() ; ++i)
			{
				weights[c][i] = inWeights[idx++];
			}
		}
		model->setWeights(weights);
	}

	void set_bias(PerceptronMultiClasses *model, float *inBias)
	{
		if(!model || !inBias)
		{
			return;
		}
		std::vector<float> bias(model->getNumClasses());
		for(int c = 0 ; c < model->getNumClasses() ; ++c)
		{
			bias[c] = inBias[c];
		}
